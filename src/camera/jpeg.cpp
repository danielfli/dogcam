...
ExifEntry *exif_create_tag(ExifData *exif, ExifIfd ifd, ExifTag tag)
{
	...
	if (!entry)
		std::cout << "failed to allocate EXIF entry" << std::endl;
	entry->tag = tag;
...
//options->thumb_quality ersetzt durch true
		if (true)
		{
			...
			//ToDo: Hardcoded Thumbnail dimensions are 320 x 240
			// Thumbnail size 7367
			// JPEG size is 1176516
			// EXIF data len 266
			// if (options->verbose)
				std::cerr << "Thumbnail dimensions are " << 320 << " x " << 240
						  << std::endl;
			entry = exif_create_tag(exif, EXIF_IFD_1, EXIF_TAG_IMAGE_WIDTH);
			exif_set_short(entry->data, exif_byte_order, 320);
			entry = exif_create_tag(exif, EXIF_IFD_1, EXIF_TAG_IMAGE_LENGTH);
			exif_set_short(entry->data, exif_byte_order, 240);
			...
			int q = 0; // options->thumb_quality;
			for (; q > 0; q -= 5)
			{
				YUV_to_JPEG((uint8_t *)(mem[0].data()), info, 320,
							240, q, 0, thumb_buffer, thumb_len);
			...
			if (q <= 0)
				std::cerr << "failed to make acceptable thumbnail" << std::endl;
			   ...
			   std::string const &cam_name)
{
	...
	try
	{
		...
		create_exif_data(mem, info, metadata, cam_name, exif_buffer, exif_len,
						 thumb_buffer, thumb_len);

		// Make the full size JPEG (could probably be more efficient if we had
		// YUV422 or YUV420 planar format).

		// ToDo: options->quality --> 92 defult wert ; options->restart --> 0 
		jpeg_mem_len_t jpeg_len;
		YUV_to_JPEG((uint8_t *)(mem[0].data()), info, info.width, info.height, 92,
